...
Logic verified in analysis_patterns.py:
  ...
  - Brand: Theme IN ('Branding','Log360 - Branding')
...
WHERE CampaignCountry = '{COUNTRY_ROI}'
  ...
  AND Theme NOT IN ('Branding','Log360 - Branding')
...
WHERE CampaignCountry = '{COUNTRY_ROI}'
  AND COALESCE(Source_Medium, Source___Medium) = 'google / cpc'
  AND Theme IN ('Branding','Log360 - Branding')
  AND Lead_Type = '{LEAD_TYPE}'
  AND SUBSTR(Date,1,4) IN ('2024','2025','2026')
GROUP BY 1
HAVING yr != 'other'
...
SELECT
  ...
  CASE WHEN FIRST_SRC_THEME IN ('Branding','Log360 - Branding')
...
WHERE LOWER(COMMON_COUNTRY_NAME) = '{COUNTRY_SL}'
  ...
  AND FIRST_SRC_THEME NOT IN ('Branding','Log360 - Branding')
  AND (FIRST_SRC_CAMPAIGN_TYPE != 'Display' OR FIRST_SRC_CAMPAIGN_TYPE IS NULL)
  AND Junk = 'false'
  AND PRODUCT_GROUP = 'AD_GROUP'
  AND SUBSTR(Created_Time,8,4) IN ('2024','2025','2026')
...
WHERE LOWER(COMMON_COUNTRY_NAME) = '{COUNTRY_SL}'
  AND FIRST_SRC_GRP = 'google / cpc'
  AND FIRST_SRC_THEME IN ('Branding','Log360 - Branding')
  AND (FIRST_SRC_CAMPAIGN_TYPE != 'Display' OR FIRST_SRC_CAMPAIGN_TYPE IS NULL)
  AND Junk = 'false'
  AND PRODUCT_GROUP = 'AD_GROUP'
  AND SUBSTR(Created_Time,8,4) IN ('2024','2025','2026')
...
# ── Q14: SEO — local /it/ vs global (salesleads_qt) ──────────────────────────
run("Q14 · SEO — local /it/ vs global pages by year (salesleads_qt)", f"""
SELECT
  {YR_SL} AS yr,
  CASE WHEN REGEXP_CONTAINS(LOWER(FIRST_SRC_URL_CLEANED), r'^/it(/|$)')
       THEN 'Local (/it/)' ELSE 'Global (EN)' END AS page_type,
  COUNT(DISTINCT ID)                                      AS total_leads,
  COUNT(DISTINCT IF(Conversion='converted', ID, NULL))    AS convs
FROM `{SL}`
WHERE LOWER(COMMON_COUNTRY_NAME) = '{COUNTRY_SL}'
  AND FIRST_SRC_GRP IN ('google / organic','organic / (not set)','organic')
  AND Junk = 'false'
  AND PRODUCT_GROUP = 'AD_GROUP'
  AND SUBSTR(Created_Time,8,4) IN ('2024','2025','2026')
GROUP BY 1, 2
