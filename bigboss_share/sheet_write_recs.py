"""
Write kw_ai_recommendations into the 'AI Keyword Recommendations' column of the 'May 2026' tab,
matching rows by (Product, Country, Theme). Updates ONLY that column. Other columns untouched.
Usage:  python sheet_write_recs.py test   (updates 1 row, prints response)
        python sheet_write_recs.py go      (updates all matched rows)
"""
...
TAB=os.environ.get('RECS_TAB','May 2026'); COL='AI Keyword Recommendations'

bq=cfg.bq_client()
recs={(r['product'],r['country'],r['theme']):r['cell_text'] for r in bq.query("SELECT product,country,theme,cell_text FROM `it-security-online-marketing.Google_ads_data_ajay.kw_ai_recommendations`").result()}
print(f"{len(recs)} recommendation cells loaded")
...
for sr in sheet_rows:
    ...
    elif MODE=="force" and filled:   # had old content but no longer a rec -> clear
...
def update_row(key, text):
    ...
    payload={'method':'worksheet.records.update','worksheet_name':TAB,
             'criteria':criteria,'data':json.dumps({COL:text})}
    for net in range(5):   # tolerate transient DNS/connection blips
        try:
