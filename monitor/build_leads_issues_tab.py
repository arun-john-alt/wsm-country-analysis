...
DRI -> Country -> Product -> Theme. Issue = theme's CONSOLIDATED leads run >=25% below its trailing
3-month norm (base>=3, drop>=2). Product rollup row shows the product's own trend. Google & Bing
columns show the engine split for the current month (so DRI sees which engine dragged).
Country-dependent presales/sales metric via monitor_leads_monthly. Notes col for follow-up."""
import google.auth
...
CUR='2026-05'; BASE=['2026-02','2026-03','2026-04']; YOY='2025-05'
DECLINE=25.0; BASE_FLOOR=3.0; ABS_FLOOR=2.0

DRI={'United States':'Aashiq','India':'Ajay','United Kingdom':'Ajay','Canada':'Ajay','Australia':'Ajay',
 'Region - MEA':'Indhu','Region - APAC':'Indhu','South Africa':'Elanthendral','Brazil':'Elanthendral',
 'Region - LATAM':'Elanthendral','Mexico':'Elanthendral','Spain':'Elanthendral','Israel':'Elanthendral',
 'Italy':'Kowsik','Saudi Arabia':'Kowsik','United Arab Emirates':'Kowsik','France':'Kowsik','Turkey':'Kowsik',
 'Region - Europe':'Sathish','Poland':'Sathish','Germany':'Jude','Netherlands':'Jude','Switzerland':'Jude',
 'Belgium':'Jude','Singapore':'Suganesh'}
def dri(c): return DRI.get(c,'Unassigned')
...
# --- supporting metrics for the Comments column (Google-search side; May vs 3-mo-avg) ---
_ml="','".join([CUR]+BASE)
RAW={}; CHd={}
...
for r in bq.query(f"SELECT product,country,theme,category,SUM(count) n FROM `{G}.change_events` WHERE date BETWEEN '2026-04-01' AND '2026-05-31' GROUP BY 1,2,3,4").result():
...
def comment(p,c,t):
    parts=[]   # only MATERIAL movers (vs 3-mo-avg), Google-search side
    ...
    cv,bv=_c3(p,c,t,'ctr')
    if cv is not None and bv is not None and abs((cv-bv)*100)>=1.5: parts.append(f"CTR {(cv-bv)*100:+.1f}pp")
    cv,bv=_c3(p,c,t,'sis')
    if cv is not None and bv is not None and abs((cv-bv)*100)>=3: parts.append(f"IS {cv*100:.0f}% ({(cv-bv)*100:+.0f}pp)")
    ...
    s=", ".join(parts)
    if tail: s += (" | " if s else "") + "Apr–May: " + ", ".join(tail)
...
def engcell(may,base):   # engine count + small-grey vs-3mo%, blank if none
...
heads=['DRI','Country','Product','Theme','Leads (May, YoY)','Google','Bing','3-mo avg','vs 3-mo','Comments']
...
for (c,p) in order:
    ...
    # product rollup (consolidated + engine totals)
    ...
    for k,may,base,yy in g['issues']:
        ...
        vc=ws.cell(r,5); vc.value=valpct(round(may),pctc(may,yy)); vc.fill=RED; vc.alignment=CEN   # E = YoY%
        ...
        ws.cell(r,9).value=smallpct(pctc(may,base)); ws.cell(r,9).alignment=CEN   # I = vs 3-mo
        cm=ws.cell(r,10, comment(k[0],k[1],k[2])); cm.font=CAL; cm.alignment=Alignment(wrap_text=True, vertical='top')
        r+=1
ws.freeze_panes='E2'
wb.save(OUT)
print(f"[ok] 'Leads Issues (by DRI)' + Google/Bing cols: {len(groups)} blocks, {sum(len(g['issues']) for g in groups.values())} issues")
