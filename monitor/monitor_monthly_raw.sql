...
CREATE OR REPLACE TABLE `it-security-online-marketing.Ads_data_WSM.monitor_monthly_raw` AS
WITH agmap AS (
  SELECT ad_group_id, ANY_VALUE(ad_group_name) agn FROM `it-security-online-marketing.Google_ads_data_ajay.ads_AdGroup_5419501619` GROUP BY 1
),
cmap AS (
  SELECT campaign_id, ANY_VALUE(campaign_name) cn FROM `it-security-online-marketing.Google_ads_data_ajay.ads_Campaign_5419501619` GROUP BY 1
),
tmap AS (
...
perf AS (   -- ad-group × month: SEARCH-network only (match the keyword-grain dashboard scope)
  ...
  FROM `it-security-online-marketing.Google_ads_data_ajay.ads_AdGroupBasicStats_5419501619`
...
isag AS (   -- ad-group × month IS, weighted by search impressions
  SELECT x.ad_group_id, FORMAT_DATE('%Y-%m', x.segments_date) ym,
         ...
         SUM(b.imp) AS sw
  FROM `it-security-online-marketing.Google_ads_data_ajay.ads_AdGroupCrossDeviceStats_5419501619` x
