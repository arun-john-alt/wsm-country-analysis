...
18 months (Jan 2025→Jun 2026, most-recent-first from col E). Cell = engine leads + (YoY% vs same
month & engine last year) in small grey; green/red with ±10% dead-band. Leads = country-dependent
presales/sales metric (monitor_leads_monthly). Only themes with any leads shown."""
import google.auth
...
GREEN=PatternFill('solid',fgColor='C6EFCE'); RED=PatternFill('solid',fgColor='FFC7CE'); ALLF=PatternFill('solid',fgColor='F2F2F2')
...
for j,ym in enumerate(months):
    c=ws.cell(1,MCOL0+j,lab(ym)); c.fill=HDR; c.font=HF; c.alignment=CEN; ws.column_dimensions[get_column_letter(MCOL0+j)].width=12
...
def put(r,col,v,yoy,bold=False):
    ...
    if yoy is None:
        ...
        if abs(yoy)>=10: cc.fill=GREEN if yoy>0 else RED
    cc.alignment=CEN
def yoy(cur,prior): return ((cur-prior)/prior*100) if (prior and prior>=0.5) else None

r=2
for k in order:
    if not any(T.get((*k,m),0)>0 for m in months): continue   # skip themes with no leads at all
